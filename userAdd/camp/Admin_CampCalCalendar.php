...
// 변수 $s 에 새로운 값을 넣고 새문서를 만들면, $s 가 들어와 원하는 값을 표시해 줍니다.
	...
	$select_query = 'SELECT COUNT(*) AS Cnt, restype, sDate, DAY(sDate) AS sDay, ResConfirm, SUM(ResPrice) AS price, SUM(RtnPrice) AS RtnPrice, SUM(CASE WHEN ResOptPrice = "1@전기@5000" THEN 5000 ELSE 0 END) AS opt FROM `SURF_CAMPING_SUB`
							WHERE DelUse = "N"
								AND ResConfirm IN (1, 4)
								AND (Year(sDate) = '.$Year.' AND Month(sDate) = '.$Mon.')
							GROUP BY sDate, ResConfirm, restype';
//echo $select_query;
	...
	while ($row = mysqli_fetch_assoc($result_setlist)){
		...
		if($row['ResConfirm'] == 1){
			$sumPrice = $row['price'] + $row['opt'];
			$totalPrice += $sumPrice;

			if($row['restype'] == "현장"){
				$totalSitePrice += $sumPrice;
			}else{
				$totalOnlinePrice += $sumPrice;
			}

			if($arrResCount[$row['ResConfirm']][$row['sDay']] == null){
				$arrResCount[$row['ResConfirm']][$row['sDay']] = 0;
			}
    ...
    for($r=0;$r<=$ra;$r++){
		...
		for($z=1;$z<=7;$z++){
			...
			if($ru<=0 || $ru>$s_t){ 
				...
				$adminText = "";
				if($arrResCount[1][$ru] != ""){
					$adminText .= '<br><font color="black">'.number_format($arrResCount[1][$ru])."원</font>";
				}
...
$j(document).ready(function(){
	...
	cancelFee = "<br><br>현장예약 합계 : <?=number_format($totalSitePrice)?>원";
	cancelFee += "<br><br>온라인예약 합계 : <?=number_format($totalOnlinePrice)?>원</b>";
	cancelFee += "<br><span style='font-size:12px;color:red;'>(온라인예약 수수료 : <?=number_format($totalOnlinePrice * 0.15)?>원)</span>";
