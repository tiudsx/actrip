...
<script>
function fnBusPointAnd(gubun, num, obj, pointname){
	...
	fnBusMap2(gubun, 5, num, pointname);
}

//날짜 클릭후 서핑버스 좌석 표시
function fnBusSeatAnd(gubun, num) {
	var sDate = $j("#hidSurfBus" + gubun).val();

    $j("#busSeat").load(folderBusRoot + "/And_BusRes_info.php?selDate=" + sDate + "&busNum=" + (gubun + num));

	$j("ul.tabs li").eq(3).click();
}

function fnBusSeatViewAnd(){
	if($j("#busStop").val() == "A"){
...
//서핑버스 선택시 계산
function fnSeatSelectedAnd(obj){
	...
	if($j(obj).is(':checked')){
		if(tbCnt == 0){
			var insHtml = '		<table class="et_vars exForm bd_tb " style="width:100%;margin-bottom:5px;" id="tb' + selDate + '_' + busNum + '">' +
							...
							'					</td>' +
							'					<td style="text-align:center;" onclick="fnSeatDelAnd(this, ' + obj.value + ');"><img src="/userAdd/del.jpg" width="20" /></td>' +
							'				</tr>' +
							'			</tbody>' +
							'		</table>';
...
function fnPriceSumAnd(obj, num){
	if(num == 1){
...
<div class="container" id="contenttop">
  <section>
    <aside class="left_article"><img src="https://surfenjoy.cdn3.cafe24.com/bus/andbus.jpg" alt="" width="400" height="200" class="placeholder"/> </aside>
    <article class="right_article">
		<div style="padding-left:10px;">
		<?include 'And_BusRes_SubDate.php';?>
		</div>
    </article>
  </section>
</div>
...
<div id="containerTab">
    ...
    <div class="tab_container">
        <div id="tab3" class="tab_content">
			<?include 'And_BusRes_SubTab3.php';?>
        ...
        <div id="tab4" class="tab_content">
			...
			<?include 'And_BusRes_SubInfo.php';?>
		</div>
    </div>
    <!-- .tab_container -->
</div>
