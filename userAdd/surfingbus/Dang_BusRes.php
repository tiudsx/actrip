...
$j(document).ready(function(){
	jQuery('input[cal=sbusdate_dang]').datepicker({
		minDate : new Date('2019-06-06'),
		maxDate : new Date('2019-06-06'),
		...
		beforeShowDay: function(date) {
			...
			eDateNow.setDate(eDateNow.getDate() + 80);
	...
	jQuery('input[cal=ebusdate_dang]').datepicker({
		minDate : new Date('2019-06-06'),
		maxDate : new Date('2019-06-06'),
		onClose: function (selectedDate) {
			if(selectedDate != ""){
		...
		beforeShowDay: function(date) {
			...
			if(sDate < new Date() || eDateNow < date){
				...
				if(date >= new Date('2019-07-12') && date <= new Date('2019-08-30')){
					if(date >= new Date('2019-07-14') && date <= new Date('2019-08-17')){
					if(day == 0){
							return [true, "date-sunday"];
						}else if(day == 6){
...
function fnBusPointDang(gubun, num, obj, pointname,  mapnum){
	...
	fnBusMap2(gubun, mapnum, num, pointname);
}

//날짜 클릭후 서핑버스 좌석 표시
function fnBusSeatDang(gubun, num) {
	var sDate = $j("#hidSurfBus" + gubun).val();

    $j("#busSeat").load(folderBusRoot + "/Dang_BusRes_info.php?selDate=" + sDate + "&busNum=" + (gubun + num));

	$j("ul.tabs li").eq(3).click();
}

function fnBusSeatViewDang(){
	if($j("#busStop").val() == "A"){
...
//서핑버스 선택시 계산
function fnSeatSelectedDang(obj){
	...
	if($j(obj).is(':checked')){
		if(tbCnt == 0){
			var insHtml = '		<table class="et_vars exForm bd_tb " style="width:100%;margin-bottom:5px;" id="tb' + selDate + '_' + busNum + '">' +
							...
							'					</td>' +
							'					<td style="text-align:center;" onclick="fnSeatDelDang(this, ' + obj.value + ');"><img src="/userAdd/del.jpg" width="20" /></td>' +
							'				</tr>' +
							'			</tbody>' +
							'		</table>';
...
function fnPriceSumDang(obj, num){
	if(num == 1){
		...
		$j("#totalPrice").html(commify((cnt * 0) - (totalDisCnt * 0)) + "원" + strDis);
		$j("#lastbusPrice").html((cnt * 0) - (totalDisCnt * 0));
	}
...
//서핑버스 좌석선택 삭제
function fnSeatDelDang(obj, num){
...
<div class="" style="width:100%;display:;">
	<h1 class="ngeb clear" style="font-size:16px;height:5px;"><!--i class="bg_color"></i>청시행비치 셔틀버스 예약하기--></h1>
</div>

<div class="container" id="contenttop">
  <section>
    <aside class="left_article"><img src="https://surfenjoy.cdn3.cafe24.com/bus/Dangbus.jpg" alt="" width="400" height="200" class="placeholder"/> </aside>
    <article class="right_article">
		<div style="padding-left:10px;">
		<?include 'Dang_BusRes_SubDate.php';?>
		</div>
    </article>
  </section>
</div>
...
<div id="containerTab">
    ...
    <div class="tab_container">
        <div id="tab3" class="tab_content">
			<?include 'Dang_BusRes_SubTab3.php';?>

			<?=fnInfoMemo(3, ''); //양양셔틀버스 이용안내?>
		</div>
        <div id="tab1" class="tab_content" style="line-height:0;">
            <img src="https://surfenjoy.cdn3.cafe24.com/content/res_bus01.jpg" class="placeholder2" />
            <img src="https://surfenjoy.cdn3.cafe24.com/content/res_bus02.jpg" class="placeholder2" />
            ...
            <img src="https://surfenjoy.cdn3.cafe24.com/content/res_bus04.jpg?v=2" class="placeholder2" />
        ...
        <div id="tab4" class="tab_content">
			...
			<?include 'Dang_BusRes_SubInfo.php';?>
		</div>
    </div>
    <!-- .tab_container -->
</div>
