...
<script>
//날짜 클릭후 서핑버스 좌석 표시
function fnBusSeatSol(gubun, num) {
	var sDate = $j("#hidSurfBus" + gubun).val();

    $j("#busSeat").load(folderBusRoot + "/Sol_BusRes_info.php?selDate=" + sDate + "&busNum=" + (gubun + num));

	$j("ul.tabs li").eq(3).click();
}

function fnBusSeatViewSol(){
	if($j("#busStop").val() == "A"){
...
//서핑버스 선택시 계산
function fnSeatSelectedSol(obj){
	...
	if($j(obj).is(':checked')){
		if(tbCnt == 0){
			var insHtml = '		<table class="et_vars exForm bd_tb " style="width:100%;margin-bottom:5px;" id="tb' + selDate + '_' + busNum + '">' +
							...
							'					</td>' +
							'					<td style="text-align:center;" onclick="fnSeatDelSol(this, ' + obj.value + ');"><img src="/userAdd/del.jpg" width="20" /></td>' +
							'				</tr>' +
							'			</tbody>' +
							'		</table>';
...
function fnPriceSumSol(obj, num){
	if(num == 1){
...
<div class="container" id="contenttop">
  <section>
    <aside class="left_article"><img src="http://www.solguest.com/data/extra/30807321_j4tZwuLp_EC8594ED8B80EBB284EC8AA4EC8381ED9288EC9DB4EBAFB8ECA780.jpg" alt="" width="400" height="200" class="placeholder"/> </aside>
    <article class="right_article">
		<div style="padding-left:10px;">
		<?include 'Sol_BusRes_SubDate.php';?>
		</div>
    </article>
  </section>
</div>
...
<div id="containerTab">
    ...
    <div class="tab_container">
        <div id="tab3" class="tab_content">
			<?include 'Sol_BusRes_SubTab3.php';?>
        ...
        <div id="tab4" class="tab_content">
			...
			<?include 'Sol_BusRes_SubInfo.php';?>
		</div>
    </div>
    <!-- .tab_container -->
</div>
