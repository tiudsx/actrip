...
$select_query_sub = 'SELECT a.userName, a.userPhone, a.Etc, b.* FROM 
						`SURF_BUS_MAIN` as a INNER JOIN `SURF_BUS_SUB` as b 
...
while ($row = mysqli_fetch_assoc($result_setlist)){
	echo '$j("#seat'.$row['busSeat'].'").removeClass("tab1");';

	echo '$j("#seat'.$row['busSeat'].'").attr("onclick", "fnModifyInfoDay('.$row['subintseq'].')");';
	echo '$j("#seat'.$row['busSeat'].'").addClass("tab2");';
	echo '$j("#seat'.$row['busSeat'].' input").remove();';

	echo '$j("#seat'.$row['busSeat'].'").parent().append(" <span  '.$cssColor.'></span> <br><b>'.$row["userName"].'</b><br>['.$row["sLocation"].']<br><span><a href=tel:'.$row["userPhone"].' style=cursor:text;>'.$row["userPhone"].'</a></span>");';
}
...
$select_query_sub3 = 'SELECT COUNT(*) AS Cnt, sLocation, c.ordernum FROM `SURF_BUS_SUB` as b LEFT JOIN SURF_CODE as c
							ON b.sLocation = c.codename
