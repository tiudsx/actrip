...
<script>
function fnBusPointJukdo(gubun, num, obj, pointname){
	...
	fnBusMap2(gubun, 4, num, pointname);
}

//날짜 클릭후 서핑버스 좌석 표시
function fnBusSeatJukdo(gubun, num) {
	var sDate = $j("#hidSurfBus" + gubun).val();

    $j("#busSeat").load(folderBusRoot + "/Jukdo_BusRes_info.php?selDate=" + sDate + "&busNum=" + (gubun + num));

	$j("ul.tabs li").eq(3).click();
}

function fnBusSeatViewJukdo(){
	if($j("#busStop").val() == "A"){
...
//서핑버스 선택시 계산
function fnSeatSelectedJukdo(obj){
	...
	if($j(obj).is(':checked')){
		if(tbCnt == 0){
			var insHtml = '		<table class="et_vars exForm bd_tb " style="width:100%;margin-bottom:5px;" id="tb' + selDate + '_' + busNum + '">' +
							...
							'					</td>' +
							'					<td style="text-align:center;" onclick="fnSeatDelJukdo(this, ' + obj.value + ');"><img src="/userAdd/del.jpg" width="20" /></td>' +
							'				</tr>' +
							'			</tbody>' +
							'		</table>';
...
function fnPriceSumJukdo(obj, num){
	if(num == 1){
...
//서핑버스 좌석선택 삭제
function fnSeatDelJukdo(obj, num){
...
<div class="" style="width:100%;display:;">
	<h1 class="ngeb clear" style="font-size:16px;height:25px;"><i class="bg_color"></i>죽도 서핑패키지 셔틀버스</h1>
</div>
...
<div class="container" id="contenttop">
  <section>
    ...
    <article class="right_article">
		...
		<?include 'Jukdo_BusRes_SubDate.php';?>
		</div>
    </article>
  </section>
</div>
...
<div id="containerTab">
    <ul class="tabs">
        ...
        <li rel="tab3" style="display:none;">정류장 안내</li>
        <li rel="tab2" style="display:none;">바베큐 안내</li>
        <li class="active" rel="tab4">좌석선택</li>
    </ul>
    ...
    <div class="tab_container">
        <div id="tab4" class="tab_content">
			...
			<?include 'Jukdo_BusRes_SubInfo.php';?>
		</div>
        <div id="tab3" class="tab_content">
			<?include 'Jukdo_BusRes_SubTab3.php';?>
