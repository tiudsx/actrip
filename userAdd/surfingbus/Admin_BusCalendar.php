...
$holidays = array(
	...
	"0914"=> array( "type"=> 0, "title"=> "대체휴일", "year"=> "2018" )
);
...
// 변수 $s 에 새로운 값을 넣고 새문서를 만들면, $s 가 들어와 원하는 값을 표시해 줍니다.
	...
	");
	echo "<a href='javascript:fnCalMoveAdmin(\"$p_m\", 0);' class='tour_calendar_prev'><span class='cal_ico'></span>이전</a>";
	echo ("
            <a href='javascript:fnCalMoveAdmin(\"$n_m\", 0);' class='tour_calendar_next'><span class='cal_ico'></span>다음</a>
	...
	$select_query = 'SELECT COUNT(*) AS Cnt, busDate, DAY(busDate) AS sDay, ResConfirm FROM `SURF_BUS_SUB`
				WHERE DelUse = "N"
					AND ResConfirm IN (0, 1, 2, 3)
                    AND (Year(busDate) = '.$Year.' AND Month(busDate) = '.$Mon.')
				GROUP BY busDate, ResConfirm';
	//echo $select_query;
    ...
    for($r=0;$r<=$ra;$r++){
		...
		for($z=1;$z<=7;$z++){
			...
			if($ru<=0 || $ru>$s_t){ 
				...
				$adminText = "";
				if($arrResCount[0][$ru] != ""){
					$adminText = '<font color="blue">'.$arrResCount[0][$ru]."건 예약</font>";
				}

				if($arrResCount[1][$ru] != ""){
					$adminText .= '<br><font color="black">'.$arrResCount[1][$ru]."건 확정</font>";
				}
				...
				if($arrResCount[4][$ru] != ""){
					//$adminText .= '<br><font color="black">'.$arrResCount[4][$ru]."건 환불완료</font>";
				}
				...
				echo "<td class='cal_type2'><calBox sel='$selYN' style='min-height:65px;$selYNbg' class='tour_td_block' value='$s' weekNum='$weeknum' onclick='fnPassengerAdmin(this);'><span class='tour_cal_day' $holidayChk>$ru</span><span class='tour_cal_pay'>$adminText</span></calBox></td>";
			}
		}
