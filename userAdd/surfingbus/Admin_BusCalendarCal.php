...
// 변수 $s 에 새로운 값을 넣고 새문서를 만들면, $s 가 들어와 원하는 값을 표시해 줍니다.
	...
	$select_query = '
		SELECT *, price AS total FROM (
			SELECT "" AS title, COUNT(*) AS Cnt, busDate, DAY(busDate) AS sDay, SUM(ResPrice) AS price, ResConfirm FROM SURF_BUS_SUB
							WHERE ResConfirm = 1 
								AND (Year(busDate) = '.$Year.' AND Month(busDate) = '.$Mon.')
							GROUP BY busDate
			UNION ALL
			SELECT "합계" AS title, COUNT(*) AS Cnt, LEFT(busDate, 7), "" AS sDay, SUM(ResPrice) AS price, ResConfirm FROM SURF_BUS_SUB
							WHERE ResConfirm = 1 
								AND (Year(busDate) = '.$Year.' AND Month(busDate) = '.$Mon.')
							GROUP BY LEFT(busDate, 7)
		) as a';
	//echo $select_query;
	...
	while ($row = mysqli_fetch_assoc($result_setlist)){
		$arrResCount[$row['ResConfirm']][$row['sDay']] = $row['price'];
	}
    ...
    for($r=0;$r<=$ra;$r++){
		...
		for($z=1;$z<=7;$z++){
			...
			if($ru<=0 || $ru>$s_t){ 
				...
				echo "<td class='cal_type2'><calBox sel='$selYN' style='min-height:65px;$selYNbg' class='tour_td_block' value='$s' weekNum='$weeknum'><span class='tour_cal_day' $holidayChk>$ru</span><span class='tour_cal_pay'>$adminText</span></calBox></td>";
			}
		}
