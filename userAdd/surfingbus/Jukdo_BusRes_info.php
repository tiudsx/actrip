...
if($busGubun == "Y"){
	...
	if($busNumber == 2 && $selDate < '2019-08-26'){
		$busseatnum = 45;
	}
}else{
	$busNumName = '서울행 '.$busNumber.'호차';

	if($busNumber == 1 && $selDate < '2019-08-26'){
		$busseatnum = 45;
	}
}
?>
...
.surfbtn .on2 > a {
	...
	?>
			<tr height="68">
				...
				<td style="background-image:url(http://skinnz.godohosting.com/surfenjoy/bus/bus_1.png);background-repeat:no-repeat;background-position:center;padding:2px;" valign="top"><br><span id="seat<?=$num4?>"><span><label style=""><input type="checkbox" id="chkSeat" name="chkSeat[]" onclick="fnSeatSelectedJukdo(this);" value="<?=$num4?>" style="vertical-align:-3px;" /><?=$num4?></label></span></span></td>
				<td style="background-image:url(http://skinnz.godohosting.com/surfenjoy/bus/bus_2.jpg);background-repeat:no-repeat;background-position:center;padding:2px;" valign="top"><br><span id="seat<?=$num5?>"><span><label style="color:#808080"><!--input type="checkbox" id="chkSeat" name="chkSeat[]" onclick="fnSeatSelectedJukdo(this);" value="<?=$num5?>" style="vertical-align:-3px;" /--><?=$num5?></label></span></span></td>
			</tr>
	<?
		}else{
	?>
...
<?
	while ($rowPoint = mysqli_fetch_assoc($result_setlistPoint)){
		...
		if($rowPoint['codename'] == "설악" || $rowPoint['codename'] == "동호" || $rowPoint['codename'] == "하조대" || $rowPoint['codename'] == "동산항" || $rowPoint['codename'] == "주문진" || $rowPoint['codename'] == "기사문" || $rowPoint['codename'] == "인구" || $rowPoint['codename'] == "남애3리") continue;
