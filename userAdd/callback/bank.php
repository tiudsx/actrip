...
if($bankname == "신한"){
	...
	$bankprice = str_replace(',', '', $arrSMS[3]);
	$bankprice = str_replace('입금', '', $bankprice);
	$bankprice = str_replace('원', '', $bankprice);
...
if(!$result_set){
	...
	}else if($banknum == "351-****-6271-13" && $bankname == "농협"){ //야영장 : 이준영 // $banknum == "351-****-8484-73"
		$resgubun = "camp";
	...
	if($resgubun == "yangfe"){
		$select_query = "SELECT * FROM SURF_YANGFE_MAIN WHERE ResConfirm = 0 AND userName = '$bankuser'";

		$result_setlist = mysqli_query($conn, $select_query);
		$count = mysqli_num_rows($result_setlist);

		$subTable = "SURF_YANGFE_MAIN";
		$gubunname = "양양페스티벌";

		$totalcount = $count;
		if ($totalcount == 0){ //금액, 이름이 없을 경우
			$select_query = "UPDATE `SURF_SMS` SET goodstype = '$resgubun', MainNumberList = 'none' WHERE seq = ".$seq;
			$result_set = mysqli_query($conn, $select_query);

			$to = "lud1@naver.com,hyeju@anpcomm.com";
			$arrMail = array(
				"campStayName"=> "yangbanknone"
				,"gubun"=> $gubunname
			);
			...
			if($count == 1){
				...
				$select_query = "UPDATE SURF_YANGFE_MAIN 
							   SET ResConfirm = 2
								  ,udpdate = now()
							WHERE intseq = ".$intseq." AND ResConfirm = 0;";
				$result_set = mysqli_query($conn, $select_query);
				...
				$mailcontent = '이름 : '.$bankuser.'<br>금액 : '.$bankprice.'<br>';
				

				$select_query = "UPDATE `SURF_SMS` SET goodstype = '$resgubun', MainNumberList = '$MainNumberList' WHERE seq = ".$seq;
				$result_set = mysqli_query($conn, $select_query);
				...
				$select_query = "
							UPDATE SURF_YANGFE_MAIN
							   SET ResConfirm = 1
								  ,udpdate = now()
							WHERE userName = '".$bankuser."' AND ResConfirm = 0";
	...
	if($resgubun == "surfbus" || $resgubun == "camp"){		
			...
			//====== 야영장 알림톡 발송
				...
				while ($rowSub = mysqli_fetch_assoc($resultSite)){
					...
					if($splitM == "07" || $splitM == "08"){
						$coupon = "   - 8월 25일까지 이용시 1박당 샤워쿠폰 2매드립니다. 관리실에 문의주세요.\n";
					}
		...
		$select_query = "SELECT a.shopSeq, a.userName, a.userName, a.userPhone, a.userMail, a.Etc, a.memo, b.* FROM SURF_SHOP_RES_MAIN as a INNER JOIN (SELECT MainNumber, SUM(ResPrice) as price FROM SURF_SHOP_RES_SUB WHERE ResConfirm = 0 GROUP BY MainNumber) as b 
								ON a.MainNumber = b.MainNumber 
								WHERE price = $bankprice AND userName = '$bankuser'";
		...
		if ($totalcount == 0){ //금액, 이름이 없을 경우
			...
			if($count == 1){
				...
				$select_query = "SELECT a.*, b.gubun, b.codename FROM `SURF_SHOP` a INNER JOIN SURF_CODE b ON a.cate_3 = b.seq AND a.groupcode in ('surf', 'bbq') WHERE a.intseq IN (".$shopSeq."0)";
				$result_setlist = mysqli_query($conn, $select_query);
				...
				$mailcontent = 'seq : '.$seq.'<br><br>샵seq : '.$shopSeq.'<br>샵이름 : '.$shopName.'<br>주문번호 : '.$MainNumberList.'<br>이름 : '.$bankuser.'<br>금액 : '.$bankprice.'<br>';
