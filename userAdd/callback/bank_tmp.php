...
if(!$result_set){
	...
	if($resgubun == "surfbus" || $resgubun == "camp"){		
		...
		$select_query = "SELECT a.*, b.gubun, b.codename FROM `SURF_SHOP` a INNER JOIN SURF_CODE b ON a.cate_3 = b.seq AND a.groupcode in ('surf', 'bbq') WHERE shopbankname LIKE '%".$bankname."%' AND shopbanknum = '".str_replace("-", "", $banknum)."'";
