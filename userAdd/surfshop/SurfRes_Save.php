...
if($param == "SurfShopI"){
	...
	if(!$success){
		...
		//$kakaoMsg1 = '안녕하세요! 서프엔조이입니다.\n예약하신 ['.$userName.']님 입금대기 안내입니다.\n\n서프엔조이 ['.$shopname.'] 예약정보\n  ▶ 예약번호 : '.$ResNumber.'\n  ▶ 예약자: '.$userName.'\n  ▶ 신청목록\n'.$surfshopMsg.$surfshopPrice.$etcMsg.'---------------------------------\n  ▶ 안내사항\n    - 입금 확인 후 확정처리 부탁드립니다.\n\n  ▶ 문의\n    - '.sendTel('이승철').'\n    - http://pf.kakao.com/_HxmtMxl';

		//sendKakao("surfshop1", "surfenjoy_shop2", $kakaoMsg1, $admin_tel, urlencode(encrypt(date("Y-m-d").'|'.$ResNumber.'|'.$shopseq)), $shopname, $userName, "link3");
		//==================== 카카오톡 발송 End ====================
