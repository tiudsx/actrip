...
if($param == "zsurfres"){
	...
	if($count == 0){
		echo '<script>alert("예약이 불가능한 상품입니다.");location.href="page_IzSc74?area=57";</script>';
		exit;
	}
			...
			<div class="popover-content animate-bounce-down popover-filter__option" data-popover-content="true" style="display:none;" id="areaView">
				<div class="layer-default">
					<div class="radio-list inner">
						<div class="input-radio <?=(($param == 'surfeast') ? 'input-radio_active' : '')?>"><input type="checkbox" id="like" name="sortFilter" value="on"><label for="like" onclick="location.href='/page_IzSc74?area=57';">양양</label></div>
...
$result_setlist = mysqli_query($conn, $select_query);
	...
	while ($row = mysqli_fetch_assoc($result_setlist)){
		// || ($i == 0 && $area == "")
?>
		<li area="<?=$row["seq"]?>" class="<?=(($area == $row["seq"] || ($i == 0 && $area == "")) ? 'on3' : '')?>"><a href="/page_IzSc74?area=57"><?=$row["codename"]?></a></li>
<?
	$i++;
	}
}?>
