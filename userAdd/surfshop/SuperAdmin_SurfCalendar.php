...
// 변수 $s 에 새로운 값을 넣고 새문서를 만들면, $s 가 들어와 원하는 값을 표시해 줍니다.
	...
	$select_query = 'SELECT COUNT(*) AS Cnt, a.ResDate, DAY(a.ResDate) AS sDay, a.ResConfirm FROM `SURF_SHOP_RES_SUB` as a
				WHERE a.DelUse = "N"
                    AND (Year(a.ResDate) = '.$Year.' AND Month(a.ResDate) = '.$Mon.')
				GROUP BY a.ResDate, a.ResConfirm';
    ...
    for($r=0;$r<=$ra;$r++){
		...
		for($z=1;$z<=7;$z++){
			...
			if($ru<=0 || $ru>$s_t){ 
				...
				$gubunChk = "";
				if($arrResCount[0][$ru] != ""){
					$adminText = "<font color='blue'>미입금</font>";
					$gubunChk = "0,";
				}
				...
				//$arrResCount[3][$ru] != "" || 
				if($arrResCount[4][$ru] != ""){
					$adminText .= "<br><font color='#008000'>임시취소</font>";
					$gubunChk .= "3,";
				}
				
				if($arrResCount[5][$ru] != ""){
					$adminText .= "<br><font color='black'>확정</font>";
					$gubunChk .= "5,";
				}
				
				if($arrResCount[6][$ru] != ""){
					$adminText .= "<br><font color='red'><b>환불요청</b></font>";
					$gubunChk .= "6,";
				}
				...
				echo "<td class='cal_type2'><calBox sel='$selYN' style='min-height:80px;$selYNbg' class='tour_td_block' value='$s' weekNum='$weeknum' gubunchk='$gubunChk' onclick='fnPassengerAdmin(this);'><span class='tour_cal_day' $holidayChk>$ru</span><span class='tour_cal_pay'>$adminText</span></calBox></td>";
			}
		}
