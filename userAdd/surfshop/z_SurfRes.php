<?php 
if($_REQUEST["seq"] == ""){
	echo '<script>location.href="page_IzSc74?area=57";</script>';
	exit;
}
...
$j(document).ready(function(){
	...
	if($seq == "64"){
	?>
		$j('calbox[value="2019-06-06"]').click();
	<?
	}
	?>
});
    ...
    <article class="right_article">
		<div style="padding-left:10px;">
			...
			<div id="tour_calendar" style="display: block;">
				<?include 'z_SurfRes_Calendar.php';?>
			</div>
							...
							<tr>
								...
								<td style="text-align:center;line-height:2.5;">
									...
									<span>
										...
										<select id="selPgkM" name="selPgkM" class="select">
										<?for($i=0;$i<=$sel3;$i++){?>
											<option value="<?=$i?>"><?=$i?></option>
										<?}?>
										</select>명<br>
									...
									<span>
										여 : 
										<span style="display:none;"><select class="select"><option value="0" style="color:red; background:#EEFF00;">매진</option></select></span>
										<select id="selPgkW" name="selPgkW" class="select">
										<?for($i=0;$i<=$sel4;$i++){?>
											<option value="<?=$i?>"><?=$i?></option>
										<?}?>
										</select>명
					...
					<table class="et_vars exForm bd_tb" style="width:100%;" id="tbselStay">
						...
						<tbody>
							...
							<tr>
								...
								<td style="text-align:center;line-height:2.5;">
									<input type="hidden" id="strStayDate" name="strStayDate" readonly="readonly" value="" class="itx" cal="sdate" size="7" maxlength="7">
									<select id="selStayDay" name="selStayDay" class="select">
										<?for($i=1;$i<=5;$i++){?>
