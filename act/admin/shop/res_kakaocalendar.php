...
");
if($selMonth > 202003){
	echo "<a href='javascript:fnCalMoveAdminListKakao(\"$p_m\", 0, \"$shopseq\");' class='tour_calendar_prev'><span class='cal_ico'></span>이전</a>";
}

//if($selMonth < date("Ym", strtotime($nowMonth." +3 month"))){
// if($selMonth < 202012){
	echo "<a href='javascript:fnCalMoveAdminListKakao(\"$n_m\", 0, \"$shopseq\");' class='tour_calendar_next'><span class='cal_ico'></span>다음</a>";
// }
...
for($r=0;$r<=$ra;$r++){
	...
	for($z=1;$z<=7;$z++){
		...
		if($ru<=0 || $ru>$s_t){ 
			echo "<td><span class='tour_td_block' style='min-height:60px;'><span class='tour_cal_day'>&nbsp;</span></span></td>";
		}else{
			...
			if($gubunChk == "99"){
				echo "<td><span class='tour_td_block' style='min-height:60px;'><span class='tour_cal_day' $holidayChk>$ru</span></span></td>";
			}else{
				echo "<td class='cal_type2'><calBox sel='$selYN' style='min-height:60px;$selYNbg' class='tour_td_block' value='$s' weekNum='$weeknum' gubunchk='$gubunChk' onclick='fnPassengerAdminKakao(this, $shopseq);'><span class='tour_cal_day' $holidayChk>$ru</span><span class='tour_cal_pay'>$adminText</span></calBox></td>";
			}
		}
	}
